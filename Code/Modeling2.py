...
class RFmodeling:
    ...
    def my_modeling(self, params):
        ...
        # roc_auc_list내의 값들중 0.7 이상인 값들에 대응하는 인덱스들을 모아놓은 리스트 ind_list추출
        ind_list = np.where(roc_auc_array >= 0.7)[0].tolist()
        if not ind_list:
            ind_list = np.where(roc_auc_array >= 0.6)[0].tolist()

        # ind_list에 있는 인덱스 값들에 대응하는 f1_list의 값들만 따로 추출하여 이것들중 f1_list의 최대값이 있는 인덱스 ix추출
        f1_values = f1_array[ind_list]
        ix = ind_list[np.argmax(f1_values)]

        # ix에 대응하는 acc_list, roc_auc_list, f1_list에 값 추출
        thresholds[ix]
        acc_array[ix]
        roc_auc_array[ix]
        f1_array[ix]

        return thresholds[ix], acc_array[ix], roc_auc_array[ix], f1_array[ix]
